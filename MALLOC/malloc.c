...
uint8_t __attribute__((aligned(32))) mem2base[MEM2_MAX_SIZE] __attribute__((section(".sram")));          //外部SRAM内存池
//内存管理表
uint16_t mem1mapbase[MEM1_ALLOC_TABLE_SIZE];                          //内部SRAM内存池MAP
uint16_t mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((section (".mallocmap")));  //外部SRAM内存池MAP
//内存管理参数
